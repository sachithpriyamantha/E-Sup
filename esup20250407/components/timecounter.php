...
$tsql = "select mtd_start_date, mtd_end_date, mtd_bidclose_date, mtd_tender_no from  mms_tender_details ORDER BY mtd_bidclose_date DESC LIMIT 1 ";
...
<script>
    ...
    var nowDate = new Date('<?= date('D M d Y H:i:s O') ?>');
    //var i = 0;
    ...
    var x = setInterval(function() {
        ...
        if (distance < 0) {
            ...
            // updated 
            var savebtns = document.getElementsByClassName("item-savebtn");
            for (const el of savebtns) {
                el.disabled = true;
                el.setAttribute("title", "No Opened Tender to submit! Please wait!");
            }
