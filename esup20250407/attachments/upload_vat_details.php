...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_POST['submit']) && isset($_FILES['my_imagevat'])) {
		include "../config.php";


		$img_name = $_FILES['my_imagevat']['name'];
		$img_size = $_FILES['my_imagevat']['size'];
		$tmp_name = $_FILES['my_imagevat']['tmp_name'];
		$error = $_FILES['my_imagevat']['error'];
		...
		if ($error === 0) {
			...
			if (!in_array($img_ex_lc, $allowed_exs)) {
				...
				return header("Location: ../profile.php?error5=$em");
			}
			...
			$new_img_name = uniqid("VAT-", true) . '.' . $img_ex_lc;
			$img_upload_path = '../uploads/vat_details/' . $new_img_name;
