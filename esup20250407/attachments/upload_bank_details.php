...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_POST['submit']) && isset($_FILES['my_imagebank'])) {
		...
		$img_name = $_FILES['my_imagebank']['name'];
		$img_size = $_FILES['my_imagebank']['size'];
		$tmp_name = $_FILES['my_imagebank']['tmp_name'];
		$error = $_FILES['my_imagebank']['error'];
		...
		if ($error === 0) {
			...
			if (!in_array($img_ex_lc, $allowed_exs)) {
				...
				return header("Location: ../profile.php?error4=$em");	
			}
